...
def crack_ps():
    ...
    edit_temp = {'Create config based from dropdown listEdit3': 'Adobe Photoshop CC 2017',
                 'Create config based from dropdown listEdit2': 'V7{}Photoshop-18-Win-GM',
                 'Create config based from dropdown listEdit': '18.0.0',
                 'Button2': '',
                 'Edit': r'C:\Program Files\Adobe\Adobe Photoshop CC 2018\amtlib.dll',
                 '打开(&O)Button': ''
                 }
    ...
    ps_cra.top_window()['Edit'].wait('ready', timeout=5)
    for each in ['Edit', '打开(&O)Button']:
        control_check(application=ps_cra, control=each,
                      edit_info=edit_temp[each], wait_time=5)
    sleep(2)
    ps_cra.kill()
    sleep(1)
