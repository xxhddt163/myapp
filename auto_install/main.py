...
class Setup:
    ...
    def __init__(self):
        self.step = {"QQ": ["自定义选项",
                            ...
                            "立即安装",
                            "完成安装"]}
        self.app = Application(backend='win32').start(r"C:\Users\admin\Desktop\PCQQ2020.exe")
        self.main_window = ''
        self.main()
    ...
    def check_window(self, volume1):
        """检测窗口是否已经加载"""
        while True:
            if self.main_window.window(title=volume1).wait('enabled', timeout=300):  # 等待指定按钮出现
                return self.main_window.child_window(title=volume1)
            time.sleep(1)

    def main(self):
        if self.app.window(title="腾讯QQ安装向导").wait('enabled', timeout=300):  # 等待界面出现才执行
