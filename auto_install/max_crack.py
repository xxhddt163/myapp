...
def crack_3dmax():
    cad = Application().start(r'"D:\Program Files\Autodesk\3ds Max 2014\3dsmax.exe" /Language=CHS')
    ...
    png_file = listdir(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step1'))

    for each in png_file:  # 点击许可协议到激活界面
        x, y = object_coord(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step1', each), x_add=0)
        if x and y:
            click(x, y)

    x, y = object_coord(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step2', '0.png'))  # 检测并复制申请号
    if x and y:
        tripleClick(x, y)
        hotkey('ctrl', 'c')
        key_soft = Application().start(join(getcwd(), 'app_pkg', '3DMAX2014', 'crack', 'xf-adsk64'))
        ...
        x, y = object_coord(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step2', '1.png'), x_add=0)  # 按下我具有激活码按钮
        if x and y:
            click(x, y)
            sleep(3)
            hotkey('ctrl', 'v')  # 粘贴激活码
            sleep(1)
            x, y = object_coord(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step2', '2.png'), x_add=0)  # 按下下一步按钮
            if x and y:
                click(x, y)
                x, y = object_coord(join(getcwd(), 'app_pkg', '3DMAX2014_shot', 'step2', '3.png'),
                                    x_add=0)  # 按下完成按钮
                if x and y:
                    click(x, y)
